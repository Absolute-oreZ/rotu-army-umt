...
- [x] Build Academic modules: Courses (cadet course assignment + course management), Results (per-session GPA/CGPA with tier pills, result slips), Timetables (30-min slot editor with locked lunch break, PDF uploads).
## Audit Phase 1
...
- [ ] Set canonical env vars in Vercel (Production and Preview), remove old `SPORTS_*`/`WELFARE_*` names, and run the migration copy once more after deploy.
